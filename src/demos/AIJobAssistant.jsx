...
export default function AIJobAssistant() {
    ...
    async function analyze() {
        ...
        const systemPrompt = `You are a sharp, direct career advisor for job seekers. When given a job description, you:
...
Keep responses concise, direct, and useful. Use short paragraphs. Use → for bullet points. No fluff.`;
    ...
    return (
        <div style={{ fontFamily: FONT_MONO, background: BG, height: "100vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
            <style>{`
            ...
            <div style={{ padding: "10px 16px", borderBottom: `1px solid ${BORDER}`, display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
                ...
                {phase === "chat" && (
                    <button className="reset-btn" onClick={() => { setPhase("input"); setMessages([]); setJobDesc(""); setStreamText(""); }} style={{ marginLeft: "auto" }}>↺ RESET</button>
                )}
            </div>
            ...
            {phase === "input" && (
                <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: 16, gap: 12, overflow: "auto" }} className="ai-scroll">
                    <div style={{ fontFamily: FONT_MONO, fontSize: 10, color: "#555", letterSpacing: 1, lineHeight: 1.8 }}>
                        → Paste a job description below<br />→ Get instant fit analysis + resume tips
                    </div>
                    <textarea className="ai-textarea" rows={9} placeholder="Paste the full job description here..." value={jobDesc} onChange={e => setJobDesc(e.target.value)} />
                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                        <button className="ai-btn" onClick={analyze} disabled={!jobDesc.trim() || loading}>
                            {loading ? "Analyzing..." : "Analyze →"}
                        </button>
                        <span style={{ fontFamily: FONT_MONO, fontSize: 9, color: "#333" }}>{jobDesc.length > 0 ? `${jobDesc.length} chars` : ""}</span>
                    </div>
                    <div style={{ marginTop: 4 }}>
                        ...
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                            {["Full-Stack Developer — React, Node.js, AWS, team collaboration, Agile.", "IT Field Placement — JavaScript, AI tools, Zapier automations, chatbot dev.", "Junior Developer — Python, REST APIs, MongoDB, remote, startup environment."].map((ex, i) => (
                                <button key={i} className="ai-chip" onClick={() => setJobDesc(ex)}>Example {i + 1} ↗</button>
                            ))}
                        </div>
                    </div>
                </div>
                    ...
                    <div className="ai-scroll" style={{ flex: 1, overflowY: "auto", padding: "12px 16px", display: "flex", flexDirection: "column", gap: 12 }}>
                        {messages.map((m, i) => (
                            <div key={i} className="msg-in" style={{ display: "flex", flexDirection: "column", alignItems: m.role === "user" ? "flex-end" : "flex-start" }}>
                                <div style={{ fontFamily: FONT_MONO, fontSize: 8, color: "#333", letterSpacing: 2, marginBottom: 4, textTransform: "uppercase" }}>{m.role === "user" ? "YOU" : "AI ASSISTANT"}</div>
                                <div style={{ background: m.role === "user" ? "#1A1A2E" : CARD, border: `1px solid ${m.role === "user" ? "#2A2A4E" : BORDER}`, borderLeft: m.role === "assistant" ? `2px solid ${ACCENT}` : undefined, borderRadius: 6, padding: "10px 14px", maxWidth: "92%", fontFamily: FONT_MONO, fontSize: 11, color: m.role === "user" ? "#888" : "#C8C8D8", lineHeight: 1.9, whiteSpace: "pre-wrap", wordBreak: "break-word" }}>
                                    {m.role === "user" ? (m.content.length > 120 ? m.content.slice(0, 120) + "…" : m.content) : m.content}
                                </div>
                            </div>
                        ))}
                        {loading && streamText && (
                            <div className="msg-in" style={{ display: "flex", flexDirection: "column", alignItems: "flex-start" }}>
                                ...
                                <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderLeft: `2px solid ${ACCENT}`, borderRadius: 6, padding: "10px 14px", maxWidth: "92%", fontFamily: FONT_MONO, fontSize: 11, color: "#C8C8D8", lineHeight: 1.9, whiteSpace: "pre-wrap", wordBreak: "break-word" }}>
                                    {streamText}<span style={{ color: ACCENT }}>▋</span>
                                </div>
                            </div>
                        )}
                        {loading && !streamText && (
